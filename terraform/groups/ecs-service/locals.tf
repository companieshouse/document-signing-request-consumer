...
locals {
  stack_name                 = "document-signing-request-consumer" # this must match the stack name the service deploys into
  name_prefix                = "${local.stack_name}-${var.environment}"
  global_prefix              = "global-${var.environment}"
  service_name               = "document-signing-request-consumer"
  container_port             = "8080"
  docker_repo                = "document-signing-request-consumer"
  kms_alias                  = "alias/${var.aws_profile}/environment-services-kms"
  healthcheck_path           = "/document-signing-request-consumer/healthcheck" # healthcheck path for document-signing-request-consumer
  healthcheck_matcher        = "200"
  vpc_name                   = local.stack_secrets["vpc_name"]
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "document-signing-request-consumer.env"
